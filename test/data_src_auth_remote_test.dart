...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(SignUpRequest(name: '', email: '', password: '', keepLoggedIn: false));
    registerFallbackValue(SignInRequest(email: '', password: '', keepLoggedIn: false));
  ...
  answerWhen({required Map<String, dynamic> data, required int statusCode, Map<String, dynamic> headers = const {}}) {
  ...
  //! SignIn Feature
  group('SignIn Tests :', () {
    final successRequest = SignInRequest(email: 'laeth2004alhalabe@gmail.com', password: '12345678', keepLoggedIn: true);
    final notExistEmailRequest = SignInRequest(email: 'NotExistEmail@gmail.com', password: '12345678', keepLoggedIn: false);
    final emptyRequest = SignInRequest(email: '', password: '', keepLoggedIn: false);
    test('Success test', () async {
      answerWhen(
        statusCode: 200,
        data: {
  ...
  group('Forget Password Tests :', () {
    ...
    final emailNotExistRequest = ForgetPasswordRequest(email: 'testing.000@gmail.com');
    ...
    test('waiting at least 1 min test', () async {
      ...
      answerWhen(
        statusCode: 429,
        data: {"status": false, "message": "wait couple minutes before sending new request."},
      );
      expect(
  ...
  group('Change Password Tests :', () {
    final codeNotMathedRequest = ChangePasswordRequest(code: '######', password: '12345678');
    final codeIsNotIntegerRequest = ChangePasswordRequest(code: '######', password: '12345678');
    final newPasswordLessThan8CharactersRequest = ChangePasswordRequest(code: '123456', password: '12345');
    ...
    test('new password is less than 8 characters test', () {
      ...
      expect(
        () async => await sut.changePassword(request: newPasswordLessThan8CharactersRequest),
        throwsA(isA<AuthException>()),
      );
    });
    test('empty field test', () {
